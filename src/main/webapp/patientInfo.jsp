...
<div style="height: 700px;width: 1000px;margin-left: 100px">
    <form action="/updateServlet" method="post" class="form-inline">
        <div class="form-group" style="position: relative;float: left;margin-left: 250px">
            ...
            <br>
            <label>入院时间</label>
            <input type="text" class="form-control" readonly  required  value="${patient.atime}">
        ...
        <div class="form-group" style="position: relative;float: right;margin-right: 200px">
            <div class="clear" ></div>
            <label>状态</label>
            <input type="text" class="form-control"  required  name="state" value="${patient.state}">
            <br>
            <div class="clear" style="height:30px"></div>
            <label>临床科室</label>
            ...
            <div class="clear" style="height:30px"></div>
            <label>病床床位</label>
            <input type="text" class="form-control" readonly  required  name="bed" value="${patient.bed}">
            <input type="text" style="position: absolute" class="form-control" name="bbed"     name="bed" placeholder="推荐安排到<%=nullbed%>号床位">
            <br>
            <div class="clear" style="height:30px"></div>
            <label>押金</label>
            ...
            <div class="clear" style="height:30px"></div>
            <label>需交医疗费</label>
            <input type="text" class="form-control"  readonly required  name="expenses" value="${patient.expenses}">
            <input type="text" style=" position: absolute " class="form-control" name="bexpenses"   name="addexpenses" placeholder="增加医疗费">
            <input type="text" style=" position: absolute;margin-top: 35px" class="form-control" name="cause" placeholder="原因" >
            <br>
            <div class="clear" style="height:30px"></div>
            <br>
            <label>已缴费用</label>
        ...
        <br>
        <button type="submit" class="btn btn-success" href="${pageContext.request.contextPath}/updateServlet">确定修改</button>

        <c:if test="${patient.deposit + patient.expenses <= patient.recharge}">
        <a class="btn btn-success" href="${pageContext.request.contextPath}/updateServlet?mark=2&id=${patient.id}&name=${patient.name}&sex=${patient.sex}&age=${patient.age}&del=${patient.del}&state=${patient.state}&department=${patient.department}&bed=${patient.bed}&deposit=${patient.deposit}&expenses=${patient.expenses}&recharge=${patient.recharge}">安排出院并退还多余缴费</a>&nbsp;&nbsp;
        </c:if>

        <c:if test="${patient.deposit + patient.expenses > patient.recharge}"><c:if test="${str > patient.atime}"><a class="btn btn-danger" href="${pageContext.request.contextPath}/updateServlet?mark=3&id=${patient.id}&name=${patient.name}&sex=${patient.sex}&age=${patient.age}&del=${patient.del}&state=${patient.state}&department=${patient.department}&bed=${patient.bed}&deposit=${patient.deposit}&expenses=${patient.expenses}&recharge=${patient.recharge}">请出医院</a></c:if></c:if>
